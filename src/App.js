    ...
    <div className="min-h-screen bg-[#F8F9FE] pb-32 font-sans selection:bg-blue-100">
      ...
      <header className="p-8 flex justify-between items-center max-w-md mx-auto">
        <div className="flex items-center gap-3">
          {/* EXIT BUTTON (Back to Landing Page) */}
          <button 
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-black italic uppercase tracking-tighter text-slate-900">
              STUDYLY
            </h1>
          </div>
        </div>
        ...
        {/* ADD BUTTON */}
        <button 
          onClick={() => setShowAddModal('module')} 
          className="p-2 bg-white rounded-xl border border-slate-100 shadow-sm text-blue-600 active:scale-90 transition-transform"
        >
          <PlusCircle size={22} />
        </button>
      </header>
      ...
      <main className="px-6 max-w-md mx-auto">
        {activeTab === 'Dashboard' && <Dashboard modules={modules} exams={exams} onStartFocus={() => navigateTo('Focus')} />}
        {activeTab === 'Modules'   && <Modules   modules={modules} onStartFocus={() => navigateTo('Focus')} />}
      ...
      {/* MODAL OVERLAY */}
      {showAddModal && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-8 shadow-2xl animate-in slide-in-from-bottom-10 duration-300">
            ...
            <input 
              ...
              className="w-full bg-slate-50 border-none rounded-2xl p-4 mb-6 font-bold text-slate-900 outline-none focus:ring-2 focus:ring-blue-500"
            />
            
            <button className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">
              Save to System
            </button>
          </div>
        </div>
      ...
      {/* BOTTOM NAV */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-2xl border-t border-slate-100 px-6 py-6 flex justify-between items-center pb-10 z-50">
