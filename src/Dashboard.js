...
export default function Dashboard({ modules = [], exams = [], onStartFocus, setShowAddModal }) {
  ...
  return (
    <div className="space-y-6 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-6xl mx-auto">
      
      {/* 1. MOTIVATION & EMOTIONAL HOOK - Always Full Width */}
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-6 md:p-10 rounded-[2.5rem] text-white shadow-xl shadow-blue-200">
        <p className="text-[10px] font-bold opacity-80 mb-1 italic">"Success is the sum of small efforts."</p>
        <h2 className="text-xl md:text-3xl font-black tracking-tight">You're ahead of 65% of your plan! 🎯</h2>
      ...
      {/* RESPONSIVE GRID STARTS HERE */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        
        {/* 2. NEXT ACTION - Full width on mobile, 2 columns wide on desktop */}
        <section className="md:col-span-2">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 px-2">Next Action</h3>
          <div className="bg-white border border-slate-100 p-5 md:p-8 rounded-[2rem] shadow-sm flex items-center justify-between">
            <div>
              ...
              <div className="text-slate-900 font-black text-lg md:text-2xl italic tracking-tighter">Calculus Basics</div>
              <div className="text-slate-400 text-[10px] font-bold uppercase mt-1">Study {todayGoal} today</div>
            </div>
            <button onClick={onStartFocus} className="bg-slate-900 text-white p-4 md:p-6 rounded-2xl shadow-lg active:scale-90 transition-transform">
              <Play size={20} fill="currentColor" />
            </button>
          </div>
        </section>

        {/* 3. FOCUS & STREAKS - Side-by-side on both mobile and desktop */}
        ...
        {/* 4. MODULE PROGRESS SNAPSHOT - Left side on desktop */}
        <section className="space-y-3">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-2">Module Progress</h3>
          {modules.length > 0 ? modules.slice(0, 3).map(m => (
            <div key={m.id} className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm">
              <div className="flex justify-between text-[10px] font-black mb-2 uppercase">
                <span>{m.name}</span>
                <span className="text-blue-600">{m.progress}% Syllabus</span>
              </div>
        ...
        {/* 5. URGENT EXAMS - Right side on desktop */}
        <section className="space-y-3">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-2">Upcoming</h3>
        ...
        {/* 6. PERFORMANCE INSIGHTS - Full Width on desktop */}
        <section className="bg-blue-50 p-5 md:p-8 rounded-[2.5rem] border border-blue-100 md:col-span-2">
          <div className="flex items-center gap-2 mb-2">
            <Clock size={14} className="text-blue-600" />
            <h3 className="text-[10px] font-black uppercase text-blue-900">Smart Insights</h3>
          </div>
          <p className="text-blue-800 text-[11px] md:text-sm font-bold leading-relaxed italic">
